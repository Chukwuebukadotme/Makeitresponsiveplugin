...
function validateBreakpointRanges(ranges) {
...
async function adjustFrameWidth(targetWidth) {
    try {
...
function adjustChildrenConstraints(node, originalWidth, newWidth) {
    if (!('children' in node))
        return;
    for (const child of node.children) {
        ...
        if (child.type === 'FRAME' || child.type === 'COMPONENT' || child.type === 'INSTANCE' || child.type === 'GROUP') {
            if ('children' in child) { // Double-check 'children' exists
                adjustChildrenConstraints(child, // Cast needed for recursion
...
function findNodeById(id) {
    ...
    return traverse(figma.currentPage);
}
// Function to monitor frame resize with cleanup
function monitorFrameResize(frame) {
...
function serializeNode(node) {
    const serialized = {
        nodeId: node.id,
        nodeType: node.type, // Cast to a string type, actual type is more complex
        name: node.name,
    ...
    return serialized; // Cast to the full type
}
 ...
 */
function serializeSelectedFrameToJSON() {
    const selection = figma.currentPage.selection;
    if (selection.length === 0) {
